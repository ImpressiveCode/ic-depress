...
public class JiraOnlineAdapterNodeModel extends NodeModel {
	...
	protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
		...
		while (totalIssues > builder.getNextStartingIndex()) {
			LOGGER.info("Downloaded " + builder.getNextStartingIndex()
					+ " out of " + totalIssues + " JIRA entries.");
			builder.prepareForNextBatch();
			rawSources.add(client.getIssues());
		}
		LOGGER.info("Downloaded " + totalIssues + " out of " + totalIssues
				+ " JIRA entries.");

		LOGGER.info("Transforming JIRA entries.");
		List<ITSDataType> parsedData = JiraOnlineParser
				.parseMultipleIssueBatches(rawSources, client.getUriBuilder()
						.getHostname());
	...
	private JiraOnlineAdapterUriBuilder prepareBuilder() {
		...
		if (jiraSettingsJQL.getStringValue() != null
				&& !jiraSettingsJQL.getStringValue().equals("")) {
