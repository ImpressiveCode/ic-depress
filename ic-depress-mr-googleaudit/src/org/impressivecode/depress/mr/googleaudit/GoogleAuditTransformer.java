...
public class GoogleAuditTransformer {
    ...
    public BufferedDataTable transformMethodLevel(final List<GoogleAuditEntryMethodLevel> eclipsemetricsdata,
            final ExecutionContext exec) throws CanceledExecutionException {
        BufferedDataContainer container = createDataContainer(exec);
        for (GoogleAuditEntryMethodLevel entry : eclipsemetricsdata) {
            progress(exec);
    ...
    public BufferedDataTable transformClassLevel(final List<GoogleAuditEntryClassLevel> eclipsemetricsdata, final ExecutionContext exec)
            throws CanceledExecutionException {
        BufferedDataContainer container = createDataContainer(exec);
        for (GoogleAuditEntryClassLevel entry : eclipsemetricsdata) {
            progress(exec);
    ...
    private DataRow createTableRow(final GoogleAuditEntryMethodLevel entry) {
        DataCell[] cells = getEclipseMetricsCells(entry);
        DataRow row = new DefaultRow(entry.getMethodName(), cells);
        return row;
    }

    private DataRow createTableRow(final GoogleAuditEntryClassLevel entry) {
        DataCell[] cells = getEclipseMetricsCells(entry);
        DataRow row = new DefaultRow(entry.getClassName(), cells);
        return row;
    }

    private DataCell[] getEclipseMetricsCells(final GoogleAuditEntryMethodLevel value) {
    ...
    private DataCell[] getEclipseMetricsCells(final GoogleAuditEntryClassLevel value) {
