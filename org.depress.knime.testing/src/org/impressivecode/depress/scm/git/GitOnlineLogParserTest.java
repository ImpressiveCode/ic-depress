...
public class GitOnlineLogParserTest {
    private final static String repoZipPath = GitOnlineLogParserTest.class.getResource("test_repo.zip").getPath();
    ...
    public void unpack() throws IOException, ZipException {
        ...
        File tempRepoFolder = folder.newFolder();
        zip.extractAll(tempRepoFolder.getAbsolutePath());

        repoPath = tempRepoFolder.getAbsolutePath() + File.separatorChar + ".git";
    }
    ...
    private GitCommit specificCommit() throws IOException, ParseException, NoHeadException, GitAPIException {
        ...
        for (GitCommit c : parser.parseEntries(repoPath, options("org.", Lists.newArrayList(".java"), null))) {
    ...
    public void shouldSpecificCommitFilesMatch() throws Exception {
        ...
        GitCommit specificCommit = specificCommit();
