...
public class JiraOnlineAdapterUriBuilderTest {
	...
	public void should_have_default_link_pattern() {
		...
		String expectedPattern = "https://" + HOSTNAME
	...
	@Test
	public void should_create_link_with_JQL_filter()
			throws UnsupportedEncodingException {
		String jql = "labels=metamodel";
		String actual = builder.setJQL(jql).build().toString();
		String expected = "https://" + HOSTNAME
				+ "/rest/api/latest/search?jql=labels%3Dmetamodel";
	...
	public void should_create_start_and_end_dates_with_status_created_filter() {
		String actual = builder.setDateFrom(startDate).setDateTo(endDate)
				.setDateFilterStatus(JiraOnlineAdapterUriBuilder.CREATED)
				.build().toString();
		String expected = "https://" + HOSTNAME
				+ "/rest/api/latest/search?jql="
	...
	public void should_create_start_and_end_dates_with_status_resolution_filter() {
		String actual = builder
				...
				.setDateTo(endDate)
				.setDateFilterStatus(
						JiraOnlineAdapterUriBuilder.RESOLUTION_DATE).build()
				.toString();
		String expected = "https://" + HOSTNAME
				+ "/rest/api/latest/search?jql="
	...
	private String createBothDatesStatusCreatedExpectedFilterResult() {
		...
		expectedResult
				.append(createStartDateOnlyStatusCreatedExpectedFilterResult());
		expectedResult.append("+AND+");
		expectedResult
				.append(createEndDateOnlyStatusCreatedExpectedFilterResult());
	...
	private String createBothDatesStatusResolutionExpectedFilterResult() {
		StringBuilder expectedResult = new StringBuilder();
		expectedResult
				.append(createStartDateOnlyStatusResolutionExpectedFilterResult());
		expectedResult.append("+AND+");
		expectedResult
				.append(createEndDateOnlyStatusResolutionExpectedFilterResult());
