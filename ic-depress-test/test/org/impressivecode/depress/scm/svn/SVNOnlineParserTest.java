...
public class SVNOnlineParserTest {
	...
	public void shouldSpecificCommitMessageMatch() throws Exception {
		assertEquals("Init commit for test local repository", specificCommit()
				.getMessage());
	}
	...
	public void shouldThrowFileNotFound() throws Exception {
		new SVNOnlineLogParser().parseEntries("fake_path",
				SVNParserOptions.options(null, null));
	}
	...
	public void unpack() throws IOException, ZipException {
		tempDir = File.createTempFile("temp-SVNOnlineLogParserTest-",
	...
	private SVNCommit specificCommit() throws IOException, ParseException,
			SVNException {
		this.parser = new SVNOnlineLogParser();
		this.parser.parseEntries(repoPath,
				SVNParserOptions.options("#([0-9]+)", "org."));

		for (SVNCommit c : parser.parseEntries(repoPath,
				SVNParserOptions.options("#([0-9]+)", "org."))) {
			if (c.getId().equals("2")) {
				return c;
			}
