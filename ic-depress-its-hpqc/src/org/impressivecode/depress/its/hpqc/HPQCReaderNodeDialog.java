 ...
 */
public class HPQCReaderNodeDialog extends NodeDialogPane {
    ...
    private HPQCTable m_fileDataTable = null;
    ...
    private HPQCTable m_previewDataTable = null;
    ...
    private JComponent getXLErrBox() {
        ...
        m_formulaErrPattern.setText(HPQCUserSettings.DEFAULT_ERR_PATTERN);
        ...
        SwingWorker<String[], Object> sw = new SwingWorkerWithContext<String[], Object>() {
            ...
            protected String[] doInBackgroundWithContext() throws Exception {
                ...
                if (file != null && !file.isEmpty()) {
                    ...
                    try {
                        ArrayList<String> sheetNames = HPQCTable.getSheetNames(m_workbook);
                        sheetNames.add(0, FIRST_SHEET);
                        return sheetNames.toArray(new String[sheetNames.size()]);
                    } catch (Exception fnf) {
                        NodeLogger.getLogger(HPQCReaderNodeDialog.class).error(
                                fnf.getMessage(), fnf);
    ...
    private synchronized void updateFileTable() {
        ...
        final AtomicReference<HPQCTable> dt =
                new AtomicReference<HPQCTable>(null);
        SwingWorker<String, Object> sw = new SwingWorkerWithContext<String, Object>() {
            @Override
            protected String doInBackgroundWithContext() throws Exception {
            	HPQCUserSettings s;
                try {
                    ...
                    dt.set(new HPQCTable(s, m_workbook));
                    return "Content of XL sheet: " + sheet;
                } catch (Throwable t) {
                    NodeLogger.getLogger(HPQCReaderNodeDialog.class).debug(
    ...
    private HPQCUserSettings createSettingsFromComponents()
            throws InvalidSettingsException {
    	HPQCUserSettings s = new HPQCUserSettings();
    ...
    private int getColumnNumberFromTextField(final JTextField t)
        ...
        try {
            ...
            try {
                i = HPQCTable.getColumnIndex(input.toUpperCase());
    ...
    private void transferSettingsIntoComponents(final HPQCUserSettings s) {
        ...
        if (val >= 0) {
            m_rowIDCol.setText(HPQCTable.getColLabel(val));
        } else {
            m_rowIDCol.setText("A");
        }
        val = s.getFirstColumn(); // getColLabel wants an index
        if (val >= 0) {
            m_firstCol.setText(HPQCTable.getColLabel(val));
        } else {
            m_firstCol.setText("A");
        }
        val = s.getLastColumn() + 1;
        if (val >= 1) {
            m_lastCol.setText(HPQCTable.getColLabel(val));
        } else {
            m_lastCol.setText("");
        }
        val = s.getFirstRow() + 1;
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings)
        ...
        HPQCUserSettings s = createSettingsFromComponents();
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings,
        ...
        try {
        	HPQCUserSettings s;
            try {
                s = HPQCUserSettings.load(settings);
            } catch (InvalidSettingsException e) {
                s = new HPQCUserSettings();
            }
     ...
     */
    private static HPQCUserSettings createFileviewSettings(
        ...
        HPQCUserSettings result = new HPQCUserSettings();
        ...
        HPQCTableSettings tableSettings = new HPQCTableSettings(result, wb);
    ...
    private void refreshWorkbook(final String path) {
        if (path == null) {
            ...
            m_workbook = HPQCTableSettings.getWorkbook(path);
            m_workbookPath = path;
        }
    }
