 ...
 */
public class HPQCTableSettings {

    private static final NodeLogger LOGGER = NodeLogger
            .getLogger(HPQCTableSettings.class);

    private final HPQCUserSettings m_userSettings;
     ...
     */
    public HPQCTableSettings(final HPQCUserSettings userSettings, final Workbook wb)
     ...
     * @throws InvalidFormatException If an exception occurs
     */
    public HPQCTableSettings(final HPQCUserSettings userSettings,
        ...
        m_userSettings = HPQCUserSettings.clone(userSettings);
     ...
     *
     */
    private HPQCTableSettings(final HPQCUserSettings userSettings,
     ...
     */
    private static void setMinMaxColumnAndRow(final HPQCUserSettings settings,
                                              final Workbook wb)
        ...
        if (sheetName == null || sheetName.isEmpty()) {
            sheetName = HPQCTable.getFirstSheetNameWithData(wb);
        }
        ...
        int maxRowIdx = HPQCTable.getLastRowIdx(sheet);
     ...
     * @throws InvalidFormatException
     */
    private static DataTableSpec createSpec(final HPQCUserSettings settings,
     ...
     */
    private static String[] createColHeaders(final HPQCUserSettings settings,
    ...
    private static void fillEmptyColHeaders(final HPQCUserSettings settings,
        ...
        for (int i = 0; i < colHdrs.length; i++) {
            ...
            if (colHdrs[i] == null) {
                ...
                if (settings.getKeepXLColNames()) {
                    colName =
                    		HPQCTable.getColLabel(settings.getFirstColumn()
                                    + xlOffset);
                }
    ...
    private static ArrayList<DataType> analyzeColumnTypes(
            final HPQCUserSettings settings, final Set<Integer> skippedCols,
            final Workbook wb)
    ...
    private static ArrayList<DataType> setColumnTypes(final Workbook wb,
        ...
        if (sh != null) {
            ...
            int maxRowIdx = HPQCTable.getLastRowIdx(sh);
     ...
     * @throws InvalidSettingsException if settings are invalid
     */
    private static String[] readColumnHeaders(final HPQCUserSettings settings,
            final int numOfCols, final Set<Integer> skippedCols, final Workbook wb)
            throws InvalidSettingsException {
        ...
        try {
        	...
        	HPQCUserSettings colHdrSettings = HPQCUserSettings.clone(settings);

            // this avoids endless recursion
            colHdrSettings.setKeepXLNames(true);

            // analyze the file and set the table spec
            HPQCTableSettings tableSettings =
                    new HPQCTableSettings(colHdrSettings, wb);
            ...
            HPQCTableSettings tableS =
                    new HPQCTableSettings(colHdrSettings, new DataTableSpec(
                            colSpecs), tableSettings.m_skippedCols);
            HPQCTable table = new HPQCTable(tableS, wb);
            ...
            NodeLogger.getLogger(HPQCUserSettings.class).debug(
    ...
    public static BufferedInputStream getBufferedInputStream(
        ...
        return HPQCUserSettings.getBufferedInputStream(location);
    }
    ...
    public static Workbook getWorkbook(final String path) {
        ...
        InputStream in = null;
        try {
            in = HPQCTableSettings.getBufferedInputStream(path);
    ...
    public String getSheetName(final Workbook wb) {
        ...
        if (sheetName == null || sheetName.isEmpty()) {
            if (m_firstSheetName == null) {
                try {
                    m_firstSheetName =
                    		HPQCTable.getFirstSheetNameWithData(wb);
                } catch (Exception e) {
