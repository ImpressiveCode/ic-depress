 ...
 */
class HPQCTable implements DataTable {

    private final HPQCTableSettings m_settings;

    // list of all iterators to close the source, when the table is disposed of
    private final LinkedList<WeakReference<HPQCIterator>> m_iterators;
     ...
     */
    HPQCTable(final HPQCUserSettings settings, final Workbook wb) throws InvalidSettingsException,
        ...
        m_iterators = new LinkedList<WeakReference<HPQCIterator>>();
        m_settings = new HPQCTableSettings(settings, wb);
     ...
     * @param tableSettings
     */
    public HPQCTable(final HPQCTableSettings tableSettings, final Workbook wb) {
        m_workbook = wb;
    ...
    public void dispose() {
        synchronized (m_iterators) {
            m_workbook = null;
            for (WeakReference<HPQCIterator> w : m_iterators) {
            	HPQCIterator i = w.get();
                if (i != null) {
                    i.close();
                }
            }
	...
	public RowIterator iterator() {
        try {
            synchronized (m_iterators) {
            	HPQCIterator i = new HPQCIterator(m_settings, m_workbook);
                m_iterators.add(new WeakReference<HPQCIterator>(i));
                return i;
            }
        } catch (IOException e) {
